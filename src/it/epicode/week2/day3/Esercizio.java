...
public class Esercizio {
    public static void main(String[] args) {
       ...
       Product p7 = new Product(07,"libro2","books",8.5);
        ...
        listaProdotti.stream().filter(p -> p.getCategory()=="books" && p.getPrice()>100).forEach(p-> System.out.println(p));
        listaProdotti.stream().filter(p-> p.getCategory()=="boy").map(p -> p.getPrice()-(p.getPrice()*0.10)).forEach(p -> System.out.println(p));
